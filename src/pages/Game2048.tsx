...
import { askJev2048 } from "../api/jev";
import {
  applyMove,
  continueAfterWin,
  countEmpty,
...
import { MoveIcon, MOVE_LABEL } from "../components/MoveIcon";
import "./game2048.css";
...
export default function Game2048() {
  ...
  const stepWithJev = useCallback(async (): Promise<"ok" | "stop"> => {
    ...
    try {
      const decision = await askJev2048({
        board: working.board,
        score: working.score,
        validMoves,
  ...
  return (
    <div className="page page-2048">
      <div className="atmosphere atmosphere-2048" aria-hidden="true" />
      <BackNav />
      ...
      <main className="layout">
        <section className="stage">
          ...
          <Controls
            ...
            stopAtWin={stopAtWin}
            canStep={
              game.status !== "lost" && !(game.status === "won" && stopAtWin)
            }
          ...
          {lastMove && lastConfidence !== null && (
            <p className="last-call">
              上一步：
              <strong className="last-move">
                <MoveIcon move={lastMove} size={15} />
                {MOVE_LABEL[lastMove]}
              </strong>
